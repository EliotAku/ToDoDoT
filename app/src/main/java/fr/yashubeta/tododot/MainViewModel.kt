...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // --> Modify
    fun insertTodo(todo: Todo) { vms.launch {
        repo.insertTodo(todo.apply { position = getHighestPosition()?.plus(1) ?: 0 })
    } }
    fun updateTodos(todos: List<Todo>) { vms.launch { repo.updateTodos(todos) } }
    fun updateTodo(todo: Todo) { vms.launch { repo.updateTodo(todo) } }
    fun deleteTodos(todos: List<Todo>) { vms.launch { repo.deleteTodos(todos) } }
    fun deleteTodo(todo: Todo) { vms.launch {
